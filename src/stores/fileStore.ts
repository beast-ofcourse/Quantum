  ...
  persist(
    (set, get) => ({
      ...
      openFolder: async (path) => {
        ...
        set({ loading: true, error: null, rootPath: path, rootName: nameFromRoot(path) });
        try {
      ...
      openFiles: async (paths) => {
        ...
        const requested = paths.filter((p) => typeof p === "string" && p.length > 0);
      ...
      rename: async (oldPath, newName) => {
        ...
        for (const [k, v] of Object.entries(expanded)) {
          ...
          else if (k.startsWith(oldPath + "/") || k.startsWith(oldPath + "\\")) {
      ...
      moveItem: async (sourcePath, targetDirPath) => {
        ...
        for (const [k, v] of Object.entries(expanded)) {
          let newKey = k;
          if (k === sourcePath || k.startsWith(sourcePath + "/") || k.startsWith(sourcePath + "\\")) {

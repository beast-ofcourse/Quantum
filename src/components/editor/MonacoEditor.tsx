...
export function MonacoEditor({ tabId, path, language, value }: MonacoEditorProps) {
  ...
  const effectiveValue = isLargeFile ? (ContentStore.get(path) ?? value) : value;
  ...
  const editorOptions = useMemo<monaco.editor.IStandaloneEditorConstructionOptions>(
    () => ({
      ...
      minimap: { enabled: settings.editor.minimap, scale: settings.editor.minimapScale, renderCharacters: false, maxColumn: 120 },
  ...
  const contentLen = isLargeFile ? (ContentStore.get(path)?.length ?? 0) : value.length;
  const fileSizeLabel = contentLen > 1_000_000

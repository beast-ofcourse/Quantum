...
function rangeFromLsp(r: { start: { line: number; character: number }; end: { line: number; character: number } }) {
  return {
...
export class MonacoBridge {
  ...
  constructor(monaco: any, private client: LspClient, private languageId: string) {
    this.monaco = monaco;
  }

  register(): void {
    this.client.setDiagnosticsHandler((uri: string, diagnostics: Diagnostic[]) => {
      ...
      if (model) {
        editor.setModelMarkers(
          ...
          this.languageId,
          diagnostics.map((d) => ({
            severity: d.severity === 1 ? MarkerSeverity.Error
              : d.severity === 2 ? MarkerSeverity.Warning
              : MarkerSeverity.Info,
            message: d.message,
    ...
    const getId = (kindId: number) => completionKindMap[kindId] ?? CompletionItemKind.Text;

    this.disposables.push(
      monaco.languages.registerCompletionItemProvider(this.languageId, {
        triggerCharacters: [".", "(", ","],
        provideCompletionItems: async (model: any, position: any) => {
          const result = await this.client.requestCompletions(model.uri.toString(), position);
